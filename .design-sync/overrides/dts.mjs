// forked from design-sync lib/dts.mjs — TravelMind hat eine echte Komponente,
// die auf "Manager" endet (ParticipantsManager); die Heuristik haelt solche
// Namen fuer Hilfsobjekte und streicht sie.
...
// ABWEICHUNG vom Original: `Manager` ist aus der Ausschlussliste genommen.
//
// Gemessen am 2026-08-25: `ParticipantsManager` ist in TravelMind eine ganz
// gewoehnliche React-Komponente (`export default function
// ParticipantsManager({ tripId, participants, isEditing })`), keine
// Hilfsklasse. Die Originalzeile strich sie — sie blieb im Buendel
// importierbar, verlor aber Karte UND Typvertrag, und der Typvertrag ist
// genau das, wogegen der Design-Agent programmiert.
//
// `Placements` und `Context` bleiben ausgeschlossen; die treffen hier nichts
// und sind anderswo richtig.
const AUSGESCHLOSSENE_ENDUNGEN = /(?:Placements|Context)$/;

export const isComponentName = (n) => !n.endsWith('Props') && !/^[A-Z][A-Z0-9_]+$/.test(n)
  && !AUSGESCHLOSSENE_ENDUNGEN.test(n) && !/^use[A-Z]/.test(n);
