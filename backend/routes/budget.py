...
async def get_expenses(
    ...
    result = await db.execute(
        select(Expense).where(Expense.trip_id == trip_id).order_by(Expense.date.desc()).offset(skip).limit(limit)
    )
    expenses = result.scalars().all()

    # Get participants for name enrichment
    participants_result = await db.execute(select(Participant).where(Participant.trip_id == trip_id))
...
async def _create_expense_handler(trip_id: int, expense: ExpenseCreate, db: AsyncSession, current_user: User):
    ...
    for split in expense.splits:
        if split.get("participant_id") not in participants:
            raise HTTPException(status_code=404, detail=f"Participant {split.get('participant_id')} not found")
    ...
    if expense.splits:
        ...
        if abs(total_split - expense.amount) > 0.01:
            raise HTTPException(
                status_code=400, detail=f"Splits total ({total_split}) must equal expense amount ({expense.amount})"
            )
...
async def update_expense(
    ...
    if expense.splits is not None:
        ...
        if abs(total_split - amount) > 0.01:
            raise HTTPException(
                status_code=400, detail=f"Splits total ({total_split}) must equal expense amount ({amount})"
            )
    ...
    participants_result = await db.execute(select(Participant).where(Participant.trip_id == existing_expense.trip_id))
    participants = {p.id: p.name for p in participants_result.scalars().all()}
...
async def get_budget_summary(
    ...
    # Get expenses
    expenses_result = await db.execute(select(Expense).where(Expense.trip_id == trip_id))
    ...
    for pid, name in participants.items():
        by_participant[str(pid)] = {"name": name, "paid": 0.0, "owes": 0.0, "balance": 0.0}
    ...
    return {"total_expenses": total, "by_category": by_category, "by_participant": by_participant, "currency": currency}
