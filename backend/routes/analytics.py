...
async def _accessible_trip_ids(current_user: User, db: AsyncSession):
    ...
    own = await db.execute(
        select(Trip.id).where(
            or_(Trip.owner_id == current_user.id, Trip.id.in_(shared_ids) if shared_ids else False)
        )
    )
...
async def analytics_summary(
    ...
    if not trip_ids:
        return AnalyticsSummary(
            trips=0, diary_entries=0, photos=0, travel_days=0,
            primary_currency="EUR", total_spend=0.0, spend_by_currency={},
            spend_by_category=[], photos_by_trip=[], entries_by_trip=[], trips_by_year=[],
        )
    ...
    diary_entries = (await db.execute(
        select(func.count()).select_from(DiaryEntry).where(DiaryEntry.trip_id.in_(trip_ids))
    )).scalar() or 0

    photos = (await db.execute(
        select(func.count()).select_from(Media).where(Media.trip_id.in_(trip_ids))
    )).scalar() or 0

    photos_per_trip = (await db.execute(
        select(Media.trip_id, func.count()).where(Media.trip_id.in_(trip_ids)).group_by(Media.trip_id)
    )).all()
    entries_per_trip = (await db.execute(
        select(DiaryEntry.trip_id, func.count()).where(DiaryEntry.trip_id.in_(trip_ids)).group_by(DiaryEntry.trip_id)
    )).all()
    ...
    cur_rows = (await db.execute(
        ...
        .where(Expense.trip_id.in_(trip_ids)).group_by(Expense.currency)
    )).all()
