...
async def test_create_trip_unauthenticated_demo_disabled(client: AsyncClient):
    ...
    response = await client.post("/api/trips/", json={"title": "Test Trip", "destination": "Berlin", "budget": 1000.0})
...
async def test_get_trips_pagination(client: AsyncClient, test_user, auth_headers, db_session):
    ...
    for i in range(15):
        trip = Trip(title=f"Trip {i}", destination=f"City {i}", owner_id=test_user.id)
        db_session.add(trip)
    await db_session.commit()
...
async def test_update_trip_success(client: AsyncClient, test_user, test_trip, auth_headers):
    ...
    response = await client.put(
        f"/api/trips/{test_trip.id}", json={"title": "Updated Trip Title", "budget": 2000.0}, headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_update_trip_unauthorized(client: AsyncClient, test_trip, other_auth_headers):
    ...
    response = await client.put(
        f"/api/trips/{test_trip.id}", json={"title": "Hacked Title"}, headers=other_auth_headers
    )
    assert response.status_code == 403
...
async def test_update_trip_no_auth(client: AsyncClient, test_trip):
    ...
    response = await client.put(f"/api/trips/{test_trip.id}", json={"title": "Hacked Title"})
    assert response.status_code == 401
...
async def test_user_can_only_see_own_trips(
    client: AsyncClient, test_user, other_user, test_trip, auth_headers, db_session
):
    """Test that users can only see their own trips"""
    # Create a trip for other user
    other_trip = Trip(title="Other User's Trip", destination="Rome", owner_id=other_user.id)
