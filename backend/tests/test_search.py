...
async def seeded(db_session: AsyncSession, test_user: User):
    trip = Trip(title="Portugal Roadtrip", destination="Lisbon",
                description="Along the Atlantic coast", owner_id=test_user.id)
    db_session.add(trip)
    await db_session.commit()
    await db_session.refresh(trip)

    db_session.add(DiaryEntry(
        trip_id=trip.id, author_id=test_user.id, title="Day 1",
        content="We ate amazing pasteis de nata in Belem", tags=[], photos=[],
    ))
    db_session.add(Place(
        trip_id=trip.id, name="Time Out Market", description="A famous food hall",
        latitude=38.7, longitude=-9.1, photos=[], tags=[],
    ))
    await db_session.commit()
    return trip
...
async def test_search_is_scoped_to_user(client: AsyncClient, db_session, seeded, auth_headers):
    ...
    other = User(username="bob", email="bob@example.com",
                 hashed_password=User.hash_password("bobpass123"), is_active=True)
    db_session.add(other)
    await db_session.commit()
    await db_session.refresh(other)
    db_session.add(Trip(title="Secret Lisbon trip", destination="Lisbon",
                        description="private", owner_id=other.id))
    await db_session.commit()
